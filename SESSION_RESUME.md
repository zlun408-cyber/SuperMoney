...
- 已完成第四阶段日期分组标题视觉强化
- 已完成第四阶段工具栏摘要辅助标签化
- 已完成第四阶段筛选空状态显示当前筛选标签
- 已完成第四阶段倒序场景下同日交易序号语义修正
- 已完成第四阶段日期分组标题追加日内排序提示
- 已完成认证初始化无效 refresh token 容错修复
- 已完成登录态详情页云端 watchlist 对齐修复
- 已完成交易模型基线重构（placed/effective 字段与 sipPlans 存储）
- 已完成交易计算层兼容新旧交易结构
- 已完成交易录入表单基线重构（15 点前后 / 手续费选填 / 新交易输出结构）
- 已完成交易记录列表展示新时间语义与来源信息
- 已完成定投计划基线 UI（独立板块 / 新增弹层 / 列表展示）
- 已完成定投计划最小自动生成交易记录链路
- 已完成定投计划云端持久化基线
- 已完成定投计划结束态边界处理
...
- 已完成定投计划暂停态边界处理
- 已完成第四阶段页面层聚焦回归
- 已完成第四阶段全量单测、E2E 与生产构建回归
- 已通过 `npm run test`
- 已通过 `npm run test:e2e`
- 已通过 `npm run build`
- 当前建议下一步：继续推进基金详情页交易记录与定投计划重设计，优先补齐定投自动生成更多去重边界与交易表单去手填净值
