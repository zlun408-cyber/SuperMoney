...
## 2026-03-30（交易记录与定投计划重设计：交易表单基线）
- 做了：先补表单失败测试，覆盖 `15 点前后`、手续费选填、买入/卖出输出新交易结构
- 做了：重构 `AddTransactionDialog`，新增下单时段与手续费输入，并让保存结果输出 `placedDate / placedPeriod / effectiveDate / confirmedNav / source`
- 做了：保留旧交易记录编辑回填兼容，避免历史数据进入编辑态后失真
- 做了：修正 `15 点后` 生成次日生效日时的时区偏差
- 做了：让 `TransactionList` 兼容新旧日期字段，避免编辑/新增新结构记录后列表分组报错
- 做了：执行 `npx vitest run tests/components/fund/add-transaction-dialog.test.tsx`
- 做了：执行 `npm run test`，107 个单测全部通过
- 结论：交易表单已经进入新模型过渡期，用户现在可以录入 `15 点前后` 和手续费，新增记录也会产出标准化结构
- 下一步：继续改交易记录展示，把下单时段 / 生效日 / 系统推导字段真正露到详情页列表中

## 2026-03-30（交易记录与定投计划重设计：交易列表展示新时间语义）
- 做了：先补详情页失败测试，要求标准化交易记录在列表中显示下单时段、生效日和来源
- 做了：扩展 `TransactionList`，让列表兼容新旧交易结构，并展示下单日期 + `15 点前 / 后`、生效日、来源
- 做了：保持旧交易记录的排序、筛选、分组和影响提示逻辑不回退
- 做了：执行 `npx vitest run tests/app/fund-detail-page.test.tsx`
- 做了：执行 `npm run test`，108 个单测全部通过
- 结论：交易记录列表现在已经开始表达“何时下单”和“何时生效”的区别，为后续彻底重构业务时间语义打好了展示层基础
- 下一步：继续推进定投计划模型与 UI
...
## 2026-03-30（交易重设计：定投计划基线 UI）
- 做了：先补失败测试，覆盖 `useWatchlist` 新增定投计划能力，以及基金详情页“定投计划”独立板块的新增流程
- 做了：给 `useWatchlist` 增加 `addSipPlan`，让基金维度状态能挂载 `sipPlans`
- 做了：新增 `AddSipPlanDialog` 与 `SipPlanList`，把定投计划从交易记录里拆出来，先形成独立的计划录入与展示区
- 做了：在基金详情页接入定投计划新增入口、空状态与计划列表展示，默认先支持“每月定投 + 开始/结束日期 + 执行时间 + 执行时段”
- 做了：执行 `npx vitest run tests/lib/hooks/use-watchlist.test.tsx tests/app/fund-detail-page.test.tsx`，43 个测试全部通过
- 做了：执行 `npm run test`，110 个单测全部通过
- 结论：基金详情页现在已经有独立的定投计划区，后续可以直接在这个基线上接“自动生成交易记录”和“云端持久化”
- 下一步：继续推进定投计划到交易记录的自动生成链路，并补齐登录态下的云端持久化设计

## 2026-03-30（交易重设计：定投计划自动生成交易记录基线）
- 做了：先补失败测试，覆盖定投计划到期后自动生成买入记录、避免同一次执行重复生成
- 做了：新增 `lib/funds/sip-plans.ts`，抽出最小调度逻辑：判断计划是否到期、推进 `nextExecutionAt`、生成 `source = sip_plan` 的买入记录
- 做了：在 `useWatchlist` 中接入物化逻辑，当存在净值解析器时自动把到期计划写入交易记录
- 做了：详情页先用当前估值作为临时净值解析入口，让页面层具备计划→记录的基础联动能力
- 做了：执行 `npx vitest run tests/lib/funds/sip-plans.test.ts tests/lib/hooks/use-watchlist.test.tsx tests/app/fund-detail-page.test.tsx`，46 个测试全部通过
